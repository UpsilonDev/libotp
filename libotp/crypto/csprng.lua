...
function csprng.initalize_mt_generator(seed)
	index = 0
	MT[0] = seed
	for i=1, 623 do
...
function csprng.extract_mt(min, max) -- Get one number from the Mersenne Twister.
	if index == 0 then
		generate_mt()
	end
	local y = MT[index]
...
function csprng.seed_from_mt(seed) -- seed ISAAC with numbers from the MT:
	if seed then
		mtSeeded = false
		mtSeed = seed
	end
	if not mtSeeded or (math.random(1, 100) == 50) then -- Always seed the first time around. Otherwise, seed approximately once per 100 times.
		csprng.initalize_mt_generator(mtSeed)
		mtSeeded = true
		mtSeed = csprng.extract_mt()
	end
	for i=1, 256 do
		randrsl[i] = csprng.extract_mt()
	end
end
...
function csprng.generate_isaac(entropy)
	aa = 0
	bb = 0
	cc = 0
...
function csprng.random(min, max)
	if not max then
		max = 2^32-1
	end
	if not min then
